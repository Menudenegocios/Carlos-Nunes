    ...
    <div className="max-w-7xl mx-auto space-y-16 pb-32 pt-8 px-6">
      ...
      <section className="text-center space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-gray-900 tracking-tighter leading-none max-w-4xl mx-auto">
          O Shopping do <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">seu Bairro.</span>
        </h1>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-10">
            {[
                ...
                <button 
                    ...
                    className={`flex items-center gap-2 px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-xl scale-105' : 'bg-white text-gray-500 border border-gray-100 hover:bg-gray-50'}`}
                >
      ...
      <section className="min-h-[500px]">
        {isLoading ? (
            ...
            <div className="text-center py-32 bg-white rounded-[4rem] border-2 border-dashed border-gray-100">
                ...
                <h3 className="text-2xl font-black text-gray-900">Nenhum resultado em {activeTab}</h3>
                <p className="text-gray-500 mt-2 font-medium">Tente buscar por termos mais genéricos.</p>
            </div>
        ) : (
            ...
            {activeTab === 'negocios' && (filteredItems as MarketplaceProduct[]).map(product => (
                <div key={product.id} onClick={() => setSelectedItem({...product, type: 'product'})} className="group bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col h-full cursor-pointer">
                    ...
                    <div className="p-8 flex-1 flex flex-col">
                        <h3 className="text-xl font-black text-gray-900 leading-tight mb-2 line-clamp-2">{product.name}</h3>
                        <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between">
                            <p className="text-xl font-black text-indigo-600">R$ {product.price.toFixed(2)}</p>
            ...
            {activeTab === 'eventos' && (filteredItems as any[]).map(event => (
                <div key={event.id} onClick={() => setSelectedItem({...event, type: 'event'})} className="group bg-white rounded-[2.5rem] border border-gray-100 shadow-lg overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col h-full cursor-pointer">
                    <div className="relative h-56 overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/40 shadow-sm">
                           <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">{event.type}</span>
                        </div>
                    </div>
                    <div className="p-8 flex-1 flex flex-col">
           ...
           <div className="bg-white w-full max-w-5xl h-fit max-h-[90vh] rounded-[3.5rem] shadow-2xl overflow-hidden relative flex flex-col lg:flex-row animate-scale-in">
              ...
              <div className="flex-1 p-10 md:p-16 overflow-y-auto flex flex-col">
                 <div className="flex items-center gap-4 mb-10 pb-8 border-b border-gray-100">
                    <div className="w-14 h-14 rounded-2xl bg-gray-50 flex items-center justify-center shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div>
                       ...
                       <h4 className="font-black text-gray-900 text-lg uppercase tracking-tight">{selectedItem.businessName || 'Empreendedor Menu'}</h4>
                    </div>
                 </div>
                       ...
                       {selectedItem.type === 'event' ? (
                          <>
                             <div className="p-6 bg-gray-50 rounded-3xl border border-gray-100">
                                ...
                                <p className="font-black text-gray-900 text-sm leading-tight">{selectedItem.date} <br/> às {selectedItem.time}</p>
                             </div>
                             <div className="p-6 bg-gray-50 rounded-3xl border border-gray-100">
                                ...
                                <p className="font-black text-gray-900 text-sm leading-tight">{selectedItem.location}</p>
                             </div>
                          </>
                       ) : (
                          <>
                 ...
                 <div className="mt-12 pt-8 border-t border-gray-100 flex flex-col md:flex-row gap-4">
                    ...
                    {selectedItem.type === 'product' && (
                       <Link 
                         ...
                         className="flex-1 bg-indigo-50 text-indigo-600 py-5 rounded-[1.8rem] font-black text-xs uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all border border-indigo-100 flex items-center justify-center gap-3"
                       >
