...
import { 
  ...
  Image as ImageIcon, AlignLeft, Type, Sparkles, Camera
} from 'lucide-react';
...
export const MyBlog: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPost) {
        // Mocking update by delete + create in this simple demo
        ...
        {activeSubTab === 'home' ? (
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-[3rem] p-10 md:p-16 border border-gray-100 dark:border-zinc-800 shadow-xl space-y-8 min-h-[500px]">
            <div className="flex justify-between items-center mb-8">
               ...
               <button onClick={() => handleOpenModal()} className="text-[10px] font-black text-indigo-600 dark:text-brand-primary uppercase tracking-widest flex items-center gap-2 hover:underline">
                  <Plus className="w-4 h-4" /> CRIAR NOVO AGORA
               </button>
            </div>
            
            {isLoading ? (
              <div className="text-center py-20 text-slate-400 font-bold uppercase tracking-widest animate-pulse">Carregando seus artigos...</div>
            ) : posts.length === 0 ? (
                ...
                {posts.map(post => (
                  <div key={post.id} className="p-8 bg-gray-50/50 dark:bg-zinc-800/40 rounded-[2.5rem] border border-gray-100 dark:border-zinc-800 flex flex-col md:flex-row items-center justify-between group hover:bg-white dark:hover:bg-zinc-800 transition-all hover:shadow-2xl">
                    <div className="flex items-center gap-8">
                       ...
                       <div>
                          <span className="text-[9px] font-black text-indigo-600 dark:text-brand-primary uppercase tracking-widest bg-white dark:bg-zinc-900 px-3 py-1 rounded-lg border border-gray-100 dark:border-zinc-700 mb-2 inline-block shadow-sm">{post.category}</span>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-[3.5rem] w-full max-w-4xl max-h-[90vh] shadow-2xl overflow-hidden border border-white/5 animate-scale-in flex flex-col">
                <div className="bg-[#0F172A] p-8 text-white flex justify-between items-center flex-shrink-0">
                    <div>
                      ...
                      <p className="text-[10px] font-black text-[#F67C01] tracking-widest uppercase">Gere autoridade e atraia novos clientes locais.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-10 space-y-10 scrollbar-hide">
                    <div className="grid lg:grid-cols-12 gap-10">
                       ...
                       <div className="lg:col-span-7 space-y-8">
                          <div>
                             <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1 flex items-center gap-2"><Type className="w-3 h-3" /> Título de Impacto</label>
                             <input required type="text" className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-2xl p-5 font-black text-xl italic tracking-tight dark:text-white shadow-inner focus:ring-4 focus:ring-orange-500/5 transition-all" value={blogForm.title} onChange={e => setBlogForm({...blogForm, title: e.target.value})} placeholder="Ex: 5 Dicas para escolher o melhor bolo..." />
                          </div>

                          <div className="grid grid-cols-2 gap-6">
                             <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Categoria</label>
                                <select className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-2xl p-5 font-bold dark:text-white shadow-inner" value={blogForm.category} onChange={e => setBlogForm({...blogForm, category: e.target.value})}>
                             ...
                             <div>
                                ...
                                <input required type="text" className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-2xl p-5 font-bold dark:text-white shadow-inner" value={blogForm.author} onChange={e => setBlogForm({...blogForm, author: e.target.value})} />
                             </div>
                          </div>
                          ...
                          <div>
                             ...
                             <textarea rows={2} required className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-2xl p-5 text-sm font-medium leading-relaxed dark:text-white shadow-inner resize-none focus:ring-4 focus:ring-orange-500/5 transition-all" value={blogForm.summary} onChange={e => setBlogForm({...blogForm, summary: e.target.value})} placeholder="Uma breve descrição que aparecerá na vitrine do blog." />
                          </div>
                       </div>
                       ...
                       <div className="lg:col-span-5">
                          ...
                          <div className="aspect-[4/3] bg-gray-50 dark:bg-zinc-800/40 rounded-[2.5rem] border-4 border-dashed border-gray-100 dark:border-zinc-700 relative overflow-hidden group cursor-pointer shadow-inner" onClick={() => fileInputRef.current?.click()}>
                             {blogForm.imageUrl ? (
                               <img src={blogForm.imageUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                             ) : (
                               <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-4">
                                  <Camera className="w-12 h-12" />
                                  <span className="text-[10px] font-black uppercase tracking-[0.2em]">Upload da Imagem</span>
                               </div>
                             )}
                          ...
                          <div className="mt-4 p-4 bg-indigo-50 dark:bg-indigo-950/20 rounded-2xl border border-indigo-100 dark:border-indigo-900/30 flex gap-3">
                             ...
                             <p className="text-[10px] text-indigo-700 dark:text-indigo-400 font-medium leading-relaxed">
                                <strong>Dica PRO:</strong> Use imagens reais do seu negócio ou alta qualidade do Unsplash para gerar 2x mais cliques.
                             </p>
                          </div>
                       </div>

                       {/* Conteúdo Completo (Full Width) */}
                       <div className="lg:col-span-12">
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Conteúdo do Artigo</label>
                          <textarea rows={12} required className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-[2.5rem] p-10 text-lg font-medium leading-relaxed dark:text-white shadow-inner focus:ring-8 focus:ring-orange-500/5 transition-all" value={blogForm.content} onChange={e => setBlogForm({...blogForm, content: e.target.value})} placeholder="Escreva aqui seu artigo completo. Use sua expertise para educar seu cliente local!" />
                       </div>
                    </div>
                </form>

                <div className="p-10 bg-gray-50 dark:bg-zinc-950 border-t border-gray-100 dark:border-zinc-800 flex-shrink-0">
                    <button onClick={handleSubmit} disabled={isSaving} className="w-full bg-[#F67C01] text-white font-black py-6 rounded-[2.5rem] shadow-2xl uppercase tracking-widest text-sm hover:bg-orange-600 transition-all active:scale-95 flex items-center justify-center gap-4">
