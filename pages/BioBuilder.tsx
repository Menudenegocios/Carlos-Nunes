...
export const BioBuilder: React.FC = () => {
  const { user } = useAuth();
  const [activeEditorTab, setActiveEditorTab] = useState<'home' | 'content' | 'design' | 'share'>('home');
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-20 pt-4 px-4">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                 ...
                 <p className="text-indigo-200 text-lg font-medium">Sua porta de entrada no mundo digital.</p>
              </div>
            </div>
      ...
      <div className="flex justify-center">
        <div className="flex p-2 bg-white dark:bg-zinc-900 rounded-[2rem] border border-gray-100 dark:border-zinc-800 shadow-sm overflow-x-auto scrollbar-hide max-w-full">
            {[
                ...
                <button 
                  ...
                  className={`flex items-center justify-center gap-3 px-8 py-4 rounded-[1.4rem] font-black text-xs tracking-widest transition-all whitespace-nowrap ${activeEditorTab === tab.id ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-800'}`}
                >
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
         {activeEditorTab === 'home' && (
            <SectionLanding 
                ...
                subtitle="Bio Digital Pro"
                description="Transforme seus seguidores em clientes. Crie uma página de destino otimizada para dispositivos móveis que reúne seus contatos, redes sociais e catálogo em um só lugar."
                benefits={[
                "Substitua links genéricos por uma página premium e autoral.",
                "Design totalmente customizável (cores, fontes e layout).",
                "QR Code automático para usar em cartões físicos ou mesas.",
                "Integração direta com WhatsApp e Redes Sociais.",
                "Layout responsivo focado em conversão mobile."
                ]}
                youtubeId="dQw4w9WgXcQ"
                ctaLabel="COMEÇAR EDIÇÃO"
         ...
         {activeEditorTab !== 'home' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                        ...
                        <div className="bg-white dark:bg-zinc-900 rounded-[3rem] p-10 border border-gray-100 dark:border-zinc-800 shadow-xl space-y-10 animate-fade-in">
                            <div className="flex flex-col md:flex-row gap-8 items-start">
                                <div className="relative group">
                                    <div className="w-32 h-32 rounded-[2.5rem] bg-gray-50 dark:bg-zinc-800 border-2 border-dashed border-gray-200 dark:border-zinc-700 overflow-hidden flex items-center justify-center relative shadow-sm group-hover:border-indigo-400 transition-colors">
                                ...
                                <div className="flex-1 space-y-4 w-full">
                                    <div>
                                        ...
                                        <input type="text" className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-2xl p-4 font-bold focus:ring-2 focus:ring-indigo-100 transition-all dark:text-white" value={profile.businessName || ''} onChange={e => setProfile({...profile, businessName: e.target.value})} placeholder="Seu nome profissional" />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Biografia Curta</label>
                                        <textarea rows={2} className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-2xl p-4 font-medium text-sm focus:ring-2 focus:ring-indigo-100 resize-none transition-all dark:text-white" value={profile.bio || ''} onChange={e => setProfile({...profile, bio: e.target.value})} placeholder="O que você faz?" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6 pt-8 border-t border-gray-100 dark:border-zinc-800">
                                <div className="flex justify-between items-center">
                                    <h4 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-widest">Meus Links</h4>
                                    <button onClick={() => setLinks([...links, {id: Date.now().toString(), type: 'custom', label: 'Novo Link', url: '', active: true}])} className="text-indigo-600 font-black text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 px-4 py-2 rounded-xl transition-all">
                                        <Plus className="w-4 h-4" /> Adicionar
                                    </button>
                                </div>
                                <div className="space-y-4">
                                    {links.map((link, idx) => (
                                        <div key={link.id} className="p-6 bg-gray-50 dark:bg-zinc-800 rounded-[2rem] border border-gray-100 dark:border-zinc-700 flex items-center gap-4 group">
                        ...
                        <div className="bg-white dark:bg-zinc-900 rounded-[3rem] p-10 border border-gray-100 dark:border-zinc-800 shadow-xl space-y-10 animate-fade-in">
                            <div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                                    {PRESET_THEMES.map(theme => (
                                        <button key={theme.id} onClick={() => applyTheme(theme)} className="group p-2 bg-gray-50 dark:bg-zinc-800 rounded-2xl border border-gray-200 dark:border-zinc-700 hover:border-indigo-400 transition-all text-center">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-zinc-800 rounded-2xl border border-gray-100 dark:border-zinc-700">
                                            ...
                                            <span className="text-xs font-mono font-bold uppercase dark:text-white">{bgColor}</span>
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-zinc-800 rounded-2xl border border-gray-100 dark:border-zinc-700">
                                            ...
                                            <span className="text-xs font-mono font-bold uppercase dark:text-white">{btnColor}</span>
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-zinc-800 rounded-2xl border border-gray-100 dark:border-zinc-700">
                                            ...
                                            <span className="text-xs font-mono font-bold uppercase dark:text-white">{textColor}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="pt-8 border-t border-gray-100 dark:border-zinc-800">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {FONTS.map(font => (
                                        <button key={font.id} onClick={() => setFontFamily(font.id)} className={`p-5 rounded-2xl border transition-all text-left group ${fontFamily === font.id ? 'bg-indigo-600 border-indigo-600 shadow-lg' : 'bg-gray-50 dark:bg-zinc-800 border-gray-200 dark:border-zinc-700 hover:border-indigo-400'}`}>
                ...
                <div className="lg:col-span-6 flex justify-center lg:sticky lg:top-24 h-fit">
                    <div className="relative w-[320px] h-[660px] bg-slate-950 rounded-[4.5rem] p-3 border-[12px] border-slate-900 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden">
                        <div 
                            className={`w-full h-full rounded-[3.5rem] flex flex-col items-center pt-16 px-6 text-center overflow-y-auto scrollbar-hide`}
                            style={{ 
                                background: bgColor, 
                                color: textColor, 
                        ...
                        >
                            ...
                            <p className="text-[11px] opacity-60 font-medium leading-relaxed mb-8">{profile.bio || 'Sua biografia aqui.'}</p>
                            <div className="w-full space-y-3 pb-6">
                                {links.map(l => (
                                    <div key={l.id} className="w-full py-3.5 px-6 rounded-[1.4rem] border border-white/10 font-bold text-sm tracking-tight" style={{ background: btnColor }}>
                                        {l.label}
                                    </div>
                                ))}
                            </div>
